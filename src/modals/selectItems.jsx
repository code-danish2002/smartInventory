...
const SelectItemsModal = ({ isOpen, onClose, onModalSubmit, po_id = null, po_line_item_id, line_item_name, po_item_details_id, phaseName }) => {
  ...
  const [fetchedOptions, setFetchedOptions] = useState({
    stores: false,
    sites: false,
    incharges: false, // This will be fetched when needed, key doesn't matter much
  });

  const fetchStores = async () => {
    if (fetchedOptions.stores) return; // Don't fetch again if already fetched
    setLoadingOptions(prev => ({ ...prev, stores: true }));
    try {
      const response = await api.get('/api/stores-full');
      const storeIds = response?.data?.data?.map(s => ({ value: s.store_id, label: s.store_name }));
      setOptions(prev => ({ ...prev, store_id: storeIds }));
      setFetchedOptions(prev => ({ ...prev, stores: true })); // Mark as fetched
    } catch (err) {
      console.error("Error fetching stores:", err);
      addToast(err);
    } finally {
      setLoadingOptions(prev => ({ ...prev, stores: false }));
    }
  };

  const fetchSites = async () => {
    if (fetchedOptions.sites) return; // Don't fetch again if already fetched
    setLoadingOptions(prev => ({ ...prev, sites: true }));
    try {
      const response = await api.get('/api/pops/simple');
      const popIds = response?.data?.data?.map(p => ({ value: p.pop_id, label: p.pop_name }));
      setOptions(prev => ({ ...prev, site_id: popIds }));
      setFetchedOptions(prev => ({ ...prev, sites: true })); // Mark as fetched
    } catch (err) {
      console.error("Error fetching sites:", err);
      addToast(err);
    } finally {
      setLoadingOptions(prev => ({ ...prev, sites: false }));
    }
  };

  // --- NEW: Function to fetch incharges ---
  const fetchIncharges = async () => {
    if (fetchedOptions.incharges) return; // Don't fetch again if already fetched
    setLoadingOptions(prev => ({ ...prev, incharges: true }));
    try {
      const response = await api.get('/api/user-details/simple');
      const incharges = response?.data?.data?.map(u => ({ value: u.user_id, label: u.user_name }));
      setOptions(prev => ({ ...prev, incharges: incharges }));
      setFetchedOptions(prev => ({ ...prev, incharges: true })); // Mark as fetched
    } catch (err) {
      console.error("Error fetching incharges:", err);
      addToast(err);
    } finally {
      setLoadingOptions(prev => ({ ...prev, incharges: false }));
    }
  };
  ...
  const phaseConfig = useMemo(() => {
    switch (phaseName) {
      ...
      case "Inspection":
        return { showStore: true, showSite: true, showSpare: false, showLive: false };
      default:
        // Default behavior if phaseName doesn't match known phases
        return { showStore: true, showSite: true, showSpare: false, showLive: false };
    }
  }, [phaseName]);
  ...
  // Determine if phase is inspection (where spare/live should not be available)
  const isInspectionPhase = useMemo(() => {
    return phaseName === "Inspection";
  }, [phaseName]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([itemsResponse]) => {
        ...
        setLoading(false); // Items loaded, hide main loading
      })
      .catch(err => {
        console.error(err);
        addToast(err);
    ...
    // Reset fetched options when modal opens
    if (isOpen) {
      setFetchedOptions({ stores: false, sites: false, incharges: false });
    }
  ...
  const isEnabled = (thisStatus, phase) => {
    return thisStatus === "Ready for Dispatch" ||
      thisStatus === "Received at Store" ||
      thisStatus === "Received at Site" ||
      thisStatus === "Receive Reject at Store" ||
      thisStatus === "Receive Reject at Site" ||
      thisStatus === "Site Spare Inventory" ||
      ...
      if (selectAction === 'toggle_all') {
        ...
        if (type === 'store') {
          ...
          if (!allSelected) {
            ...
            if (!fetchedOptions.stores) {
              fetchStores(); // Fetch stores if not already fetched
            }
          }
        } else if (type === 'site') {
          ...
          if (!allSelected) {
            ...
            if (!fetchedOptions.sites) {
              fetchSites(); // Fetch sites if not already fetched
            }
          }
        } else if (type === 'spare') {
          // Clear other types that are allowed for this phase
        ...
        if (type === 'store') {
          ...
          if (!exists && !fetchedOptions.stores) {
            fetchStores(); // Initiate fetch, don't need to wait
          }
          ...
          if (!exists && !fetchedOptions.sites) {
            fetchSites(); // Initiate fetch, don't need to wait
          }
  ...
  const handleSelectChange = (group, field, lineItemId, detailId) => async (option) => {
    ...
    if ((field === 'store_id' || field === 'site_id') && option && !fetchedOptions.incharges) {
      await fetchIncharges(); // Wait for fetch to complete before updating state
    }
    setItemData(prev => ({
      ...prev,
  ...
  const handleSubmit = () => {
    ...
    const submitData = { ...itemData, po_id };
          ...
          <div className="bg-white w-full max-w-6xl max-h-full overflow-auto rounded-xl shadow-2xl p-4 sm:p-8 relative transform transition-all duration-300">
                          ...
                          <li key={item.po_item_details_id}
                            ...
                            {thisEnabled ? (
                              <div className={`sm:col-span-2`}>
                                ...
                                <Select
                                  options={
                                    isStore ? options.store_id :
                                      isSite ? options.site_id :
                                        isSpare ? options.store_id : options.site_id
                                  }
                                  isClearable
                                  isDisabled={!(isStore || isSite)}
                                  placeholder={
                                    isStore ? (loadingOptions.stores ? 'Loading...' : 'Select Store') :
                                      isSite ? (loadingOptions.sites ? 'Loading...' : 'Select POP') :
                                        (isSpare || isLive) ? 'Not Required' : 'Select Phase'
                                  }
                                  value={
                                    isStore ? options.store_id.find(o => o.value === storeEntry.store_id) :
                                      isSite ? options.site_id.find(o => o.value === dispatchEntry.site_id) :
                                        isSpare ? options.store_id.find(o => o.value === spareEntry.store_id) :
                                          options.site_id.find(o => o.value === liveEntry.site_id) || null
                                  }
                                  onChange={async (option) => { // Use async here
                                    // --- NEW: Fetch options based on selection ---
                                    if (isStore && option && !fetchedOptions.stores) {
                                      await fetchStores(); // Wait for fetch to complete
                                    }
                                    if (isSite && option && !fetchedOptions.sites) {
                                      await fetchSites(); // Wait for fetch to complete
                                    }
                                    ...
                                    handleSelectChange(
                                      ...
                                      isStore || isSpare ? 'store_id' : 'site_id',
                            ...
                            {thisEnabled ? (
                              <div className={`sm:col-span-2`}> {/* Ensure 2 column span */}
                                ...
                                <Select
                                  ...
                                  placeholder={
                                    isStore ? (loadingOptions.incharges ? 'Loading...' : "Select Incharge") :
                                    isSite ? (loadingOptions.incharges ? 'Loading...' : "Select Receiver") :
                                    (isSpare || isLive) ? "Not Required" : "Select Phase"
                                  }
                                  value={options.incharges.find(o => o.value === (
                                    isStore ? storeEntry.store_incharge_user_id :
                                      isSite ? dispatchEntry.site_incharge_user_id :
                                        isSpare ? spareEntry.store_incharge_user_id : liveEntry.site_incharge_user_id
                                  )) || null}
            ...
            <div className="mt-8 pt-4 border-t flex flex-col sm:flex-row justify-between items-center">
              ...
              <div className="flex gap-3 w-full sm:w-auto justify-end">
                ...
                <button
                  className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2 rounded-lg shadow-md hover:shadow-lg transition duration-150 ease-in-out disabled:opacity-50"
                  onClick={handleSubmit}
                  disabled={!isFormValid} // --- Enable validation ---
                >
                  Submit
                </button>
              </div>
