...
export default function ErrorModal({ showErrorModal, setShowErrorModal, submitError = [] }) {
  ...
  return (
    <div
      className="modal-overlay z-[9999] items-start py-10"
      role="dialog"
      aria-modal="true"
    >
      {/* Centering container — makes the inner modal size to content but never exceed viewport */}
      <div
        className="inline-block align-top bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-slate-800
                   overflow-hidden w-auto max-w-[95vw] md:max-w-[80vw] lg:max-w-[70vw]
                   min-w-[320px]"
        style={{ boxShadow: "0 12px 30px rgba(0,0,0,0.18)" }}
      >
        {/* Header */}
        <div className="flex items-center justify-between gap-4 px-6 py-4 border-b border-gray-200 dark:border-slate-800">
          <div className="flex items-center gap-3">
            {/* icon */}
            <div className="flex items-center justify-center h-10 w-10 rounded-full bg-red-50 dark:bg-red-900/30">
              <svg
            ...
            <div>
              <h3 className="text-lg font-semibold text-red-600 dark:text-red-500">Submission Errors</h3>
          ...
          <button
            ...
            className="p-2 rounded-md hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-500 dark:text-slate-400 focus:outline-none transition-colors"
          >
        ...
        <div className="px-6 py-4">
          <div
            className="max-h-[70vh] overflow-auto rounded-lg p-3 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800
                       text-gray-700 dark:text-slate-400 space-y-3 shadow-inner"
          ...
          >
            <ul className="space-y-4">
              {uniqueErrors.length === 0 ? (
                <li className="text-sm text-gray-600 dark:text-slate-500">No errors to show.</li>
              ) : (
                uniqueErrors.map((error, index) => (
                  <li key={index} className="text-sm">
                    <div className="flex items-start gap-3">
                      <span className="mt-0.5 inline-flex h-5 w-5 items-center justify-center rounded-full bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400 text-xs font-medium">
                        !
                      </span>
                      ...
                      <div className="flex-1">
                        <div className="font-medium text-gray-800 dark:text-slate-200">{/* brief title or same text */}</div>
                        <p className="mt-1 text-xs text-gray-500 dark:text-slate-400 break-words whitespace-normal">
                          {error}
                        </p>
                      </div>
                    </div>
