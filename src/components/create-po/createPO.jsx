...
import { validateForm } from '../../utils/validatePoForm.js';
...
const CreatePO = () => {
  ...
  const myElements = [
    ...
    { name: 'inspected_byID', label: 'Inspection Authority', type: 'select', isDisabled: false, isRequired: true, placeholder: 'in options', },
  ...
  useEffect(() => {
    api.get('/api/poNumberData')
      .then(response => {
        const poOptions = response.data.data.map(po => ({
          value: po.erp_po_id,
          label: po.po_number,
        }));
        setOptions(prev => ({ ...prev, erp_po_id: poOptions }));
      })
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([userDetailsRes]) => {
        ...
        setOptions(prev => ({ ...prev, inspected_byID: inspectionAuthorityOptions, }));
      })
  ...
  useEffect(() => {
    if (poDetails.inspected_byID) {
      setPoDetails({ ...poDetails, inspected_by: options.inspected_byID.find(option => option.value === poDetails.inspected_byID)?.label });
    }
  }, [poDetails.inspected_byID, poDetails.inspected_by]);

  const handleConfirmSubmit = async (e, lineItems, esign) => {
    e.preventDefault();
    setFormLoading(true);
    const signing_params = await generateDevSigningParams();
    //console.log('signing params', signing_params);

    // Validate form
    const errors = validateForm(poDetails, assigneeUserId, lineItems);
    ...
    const apiBody = {
      ...
      po_date_of_issue: poDetails.date_of_inspection ? `${poDetails.po_date_of_issue}` : currentDateTime,
      po_created_byID: assigneeUserId || null,
      inspected_byID: poDetails.inspected_byID || null,
    ...
    const pdfBody = {
      ...
      tender_number: poDetails.tender_number,
      date_of_issue: poDetails.date_of_inspection ? `${poDetails.date_of_inspection}` : currentDateTime,
      inspection_authority_user_name: poDetails.inspected_by || null,
      assignee_name: assigneeName || poDetails?.po_created_by || null,
      po_line_items: lineItems.map(item => ({
        line_item_name: item.line_item_name || '',
        ...
        po_item_details: item.items.map(subItem => ({
          ...
          item_make: subItem.item_make || '',
          item_model: subItem.item_model || '',
          item_part: subItem.item_part || '',
          item_serial_number: subItem.item_serial_number || [],
        }))
      }))
    }
    const method = task === 'Create' ? 'post' : 'put';
    const url = task === 'Create' ? '/api/pos' : `/api/pos-correction/${poDetails?.po_id}`;
    const params = task === 'Create' ? {} : { po_id: poDetails?.po_id };
    console.log('API Call:', url, method, apiBody);

    await api[method](url, apiBody, params).then((response) => {
      //generatePDF(pdfBody, { signerName: `${user.first_name} ${user.last_name}` })
      generatePDF(pdfBody, signing_params)
      addToast(response)
      window.history.back();
    }).catch((error) => {
      addToast(error)
        ...
        <div className="flex justify-center my-2 px-4">
          <div className="w-full">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-semibold">{task} Inspection</h2>
            </div>
