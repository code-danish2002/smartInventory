...
import { MyInput, MySelect } from '../formElements/elements.jsx';
...
const AddSubItem = ({ subItemName, operation, defaultValues, onCancel, AfterAction }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-wrap gap-4 w-full mb-6 p-4 border border-gray-300 dark:border-slate-700 rounded-lg bg-gray-50 dark:bg-slate-800/50">
            {elements.map((element, index) => (
                <div key={index} className="flex flex-col gap-2 mb-4">
                    <label className="block text-sm font-medium text-gray-900 dark:text-slate-300">{element.label}</label>
            ...
            <div className="flex justify-end w-full">
                <button
                    type="button"
                    onClick={onCancel}
                    className="mr-2 px-4 py-2 bg-gray-200 dark:bg-slate-700 text-gray-800 dark:text-slate-200 rounded hover:bg-gray-300 dark:hover:bg-slate-600 transition"
                >
                    Cancel
                </button>
                <button
