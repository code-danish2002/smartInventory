...
const LineItemForm = ({ po, defaultValues, onSubmit, onCancel, loading, task, existingSerialNumbers }) => {
    ...
    // const MaxInspected = (lineItemId) => {
    //     const thisLineItem = lineItem?.find(item => item.po_line_item_id === lineItemId);
    //     const totalCount = thisLineItem?.quantity_offered;
    //     const alreadyInspected = thisLineItem?.items?.reduce((acc, item) => acc + (item.quantity_inspected || 0), 0);
    //     const count = totalCount - alreadyInspected;
    //     return count;
    // }
                    ...
                    >
                        ...
                        >
                            ...
                            {filteredLineItems?.map((item) => (
                                <div key={item.po_line_item_id} className={`flex flex-col gap-2 p-0.5 border rounded ${(item.quantity_offered*item.items.length) > countItems(item.items) && task !== 'Create Inspection' ? 'border-red-500' : ''}`}>
                                    <div className={`grid ${task !== 'Create Inspection' ? 'grid-cols-11' : 'grid-cols-11'} gap-1`}>
                                        ...
                                        <textarea placeholder='Enter Name' rows={1} className={`text-center col-span-2  ${item.line_item_name ? 'border-gray-200' : 'border border-1 rounded-md border-red-500'}`} value={item.line_item_name} readOnly={!item.isNew} onChange={(e) => handleLineItemChange(item.po_line_item_id, e.target.value, 'line_item_name')} />
                                        ...
                                        <textarea placeholder='Description' rows={1} className={`text-center col-span-2 ${item.description ? 'border-gray-200' : 'border border-1 rounded-md border-red-500'}`} value={item.description} readOnly={!item.isNew} onChange={(e) => handleLineItemChange(item.po_line_item_id, e.target.value, 'description')} />
                                        ...
                                        <input type="number" placeholder='Enter Qty Offered' className={`text-center ${item.selected && !item.quantity_offered ? 'border border-1 rounded-md border-red-500' : 'border-gray-200'}`} value={item.quantity_offered} readOnly={!item.isNew && task !== 'Create Inspection'} onChange={(e) => handleLineItemChange(item.po_line_item_id, e.target.value, 'quantity_offered')} />
