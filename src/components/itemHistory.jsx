...
const ItemHistory = ({ data = [] }) => {
  ...
  if (!Array.isArray(data) || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-10 text-gray-400 dark:text-slate-500 bg-gray-50 dark:bg-slate-800/50 rounded-xl border-2 border-dashed border-gray-200 dark:border-slate-700 transition-colors duration-500">
        <PackageX size={48} className="mb-4 opacity-20 dark:opacity-40" />
  ...
  const statusDetails = {
    "PO Initiated": { color: "bg-blue-500", icon: FileText, lightColor: "text-blue-600 bg-blue-50 dark:text-blue-400 dark:bg-blue-900/30" },
    "Inspection in Progress": { color: "bg-yellow-500", icon: ClipboardCheck, lightColor: "text-yellow-600 bg-yellow-50 dark:text-yellow-400 dark:bg-yellow-900/30" },
    "Inspection Report Submitted": { color: "bg-teal-500", icon: CloudUpload, lightColor: "text-teal-600 bg-teal-50 dark:text-teal-400 dark:bg-teal-900/30" },
    "Inspection Approved": { color: "bg-green-500", icon: CheckCircle2, lightColor: "text-green-600 bg-green-50 dark:text-green-400 dark:bg-green-900/30" },
    "Correction Required": { color: "bg-orange-500", icon: FilePenLine, lightColor: "text-orange-600 bg-orange-50 dark:text-orange-400 dark:bg-orange-900/30" },

    //Dispatches
    "Item Dispatched from Vendor": { color: "bg-indigo-500", icon: Truck, lightColor: "text-indigo-600 bg-indigo-50 dark:text-indigo-400 dark:bg-indigo-900/30" },
    "Item Dispatched from Store": { color: "bg-indigo-500", icon: Truck, lightColor: "text-indigo-600 bg-indigo-50 dark:text-indigo-400 dark:bg-indigo-900/30" },
    "Item Dispatched from Site": { color: "bg-indigo-500", icon: Truck, lightColor: "text-indigo-600 bg-indigo-50 dark:text-indigo-400 dark:bg-indigo-900/30" },
    "Item Dispatched from OEM Spare": { color: "bg-indigo-500", icon: Truck, lightColor: "text-indigo-600 bg-indigo-50 dark:text-indigo-400 dark:bg-indigo-900/30" },
    "Item Dispatched from Live": { color: "bg-indigo-500", icon: Truck, lightColor: "text-indigo-600 bg-indigo-50 dark:text-indigo-400 dark:bg-indigo-900/30" },


    //Receives
    "Item Received at Store": { color: "bg-emerald-500", icon: FaDolly, lightColor: "text-emerald-600 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-900/30" },
    "Item Received at Site": { color: "bg-emerald-500", icon: FaTruckLoading, lightColor: "text-emerald-600 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-900/30" },
    "Item Received at OEM Spare": { color: "bg-emerald-500", icon: Icons.Dolly, lightColor: "text-emerald-600 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-900/30" },
    "Item Received at Live": { color: "bg-emerald-500", icon: Icons.Dolly, lightColor: "text-emerald-600 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-900/30" },

    "Item in Spare Inventory": { color: "bg-teal-500", icon: FaTools, lightColor: "text-teal-600 bg-teal-50 dark:text-teal-400 dark:bg-teal-900/30" },
    "Item Commissioned": { color: "bg-teal-500", icon: RiRouterFill, lightColor: "text-teal-600 bg-teal-50 dark:text-teal-400 dark:bg-teal-900/30" },

    //Rejects
    "Item Rejected at Store": { color: "bg-red-500", icon: PackageX, lightColor: "text-red-600 bg-red-50 dark:text-red-400 dark:bg-red-900/30" },
    "Item Rejected at Site": { color: "bg-red-500", icon: PackageX, lightColor: "text-red-600 bg-red-50 dark:text-red-400 dark:bg-red-900/30" },

    //RMAs
    "RMA Generated": { color: "bg-purple-500", icon: Clock, lightColor: "text-purple-600 bg-purple-50 dark:text-purple-400 dark:bg-purple-900/30" },
    "Dispatch Initiated": { color: "bg-purple-500", icon: Truck, lightColor: "text-purple-600 bg-purple-50 dark:text-purple-400 dark:bg-purple-900/30" },
    "Item Received": { color: "bg-purple-500", icon: CheckCircle2, lightColor: "text-purple-600 bg-purple-50 dark:text-purple-400 dark:bg-purple-900/30" },

    //Others
    "Default": { color: "bg-gray-500", icon: Box, lightColor: "text-gray-600 bg-gray-50 dark:text-slate-400 dark:bg-slate-800/50" },
  };

  const sortedData = [...data].sort((a, b) => Date.parse(a?.created_at) - Date.parse(b?.created_at));

  return (
    <div className="w-full max-w-full max-h-[80vh] bg-slate-50 dark:bg-[#0b1120] overflow-y-auto px-3 py-6 md:p-8 lg:p-12 font-sans transition-colors duration-500">
      <style>{`
            ...
            >
              ...
              {!isLastOverall && (
                ...
                `}>
                  ...
                  <div className={`absolute w-6 h-6 bg-white dark:bg-slate-800 border border-blue-100 dark:border-blue-900/30 rounded-full text-blue-600 dark:text-blue-400 shadow-md flex items-center justify-center z-20 transform
              ...
              <div className="absolute top-0 left-0 w-full bg-white dark:bg-slate-800/40 border border-gray-100 dark:border-white/5 rounded-[2rem] p-6 shadow-sm dark:shadow-2xl backdrop-blur-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 z-10 group h-auto max-h-[200px] hover:max-h-[500px]">
                {/* Floating Icon */}
                ...
                <div className="mt-6 text-center">
                  ...
                  <h4 className="font-extrabold text-gray-800 dark:text-slate-100 text-sm leading-tight h-10 flex items-center justify-center px-2">
                    {item.item_status}
                  </h4>

                  {/* Date Badge */}
                  <div className="mt-3 mb-1 flex items-center justify-center gap-1.5 text-[10px] font-bold text-gray-400 dark:text-slate-500">
                  ...
                  <div className="hidden group-hover:block transition-all duration-500">
                    <div className="mt-4 pt-4 border-t border-gray-100 dark:border-white/5 space-y-2 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                      <div className="flex items-center justify-center gap-2 text-xs text-gray-500 dark:text-slate-400">
                        <User size={12} className="text-gray-300 dark:text-slate-600" />
                      ...
                      <div className="flex items-center justify-center gap-2 text-xs text-gray-400 dark:text-slate-500">
                        <MapPin size={12} className="text-red-300 dark:text-red-900/40" />
              ...
              <div className="absolute -bottom-4 -right-2 w-8 h-8 rounded-full bg-blue-600 border-4 border-white dark:border-slate-800 shadow-lg flex items-center justify-center text-[10px] font-black text-white z-20">
                {index + 1}
              </div>
            </div>
          );
