...
export const validateInspectionForm = (inspectionDetails,) => {
  ...
  if (!inspectionDetails.po_number?.toString().trim()) {
    //errors.push('PO Number is required.');
    inputErrors.erp_po_id = 'Please Select a PO First.';
    return { errors, inputErrors };
  }
  if (!inspectionDetails.purchaser_id) {
    errors.push('Inspector ID is required.');
...
export const validateAddDataForm = (inspectionDetails,) => {
  ...
  // — At least one line item —
  const thisLineItem = Array.isArray(lineItems) ? lineItems : [];
  if (thisLineItem.length < 1) {
    errors.push('At least one line item is required.');
  }

  // — Per-line-item checks —
  thisLineItem.forEach((li, idx) => {
    const lineNum = idx + 1;
    if (!li.line_item_name?.toString().trim()) {
      errors.push(`In Line Number "${li.line_number}": name is required.`);
    }
    if (typeof li.total_quantity !== 'number' || li.total_quantity <= 0) {
      errors.push(`In Line Number "${li.line_number}": total_quantity must be > 0.`);
    }

    if (typeof li.quantity_offered !== 'number' || li.quantity_offered <= 0) {
      errors.push(`In Line Number "${li.line_number}": quantity_offered must be > 0.`);
    }

    if (!li.warranty_start) {
      errors.push(`In Line Number "${li.line_number}": warranty is required.`);
    }

    // must have at least one detail
    if (!Array.isArray(li.po_item_details) || !li.po_item_details.length) {
      errors.push(`In Line Number "${li.line_number}": at least one item detail is required.`);
    } else {
      // detail-level required fields
      li.po_item_details.forEach((sub, j) => {
        const detNum = j + 1;
        if (!sub.item_type_id) {
          errors.push(`In Line Number "${li.line_number}", detail ${detNum}: item type is required.`);
        }
        if (!sub.item_make_id) {
          errors.push(`In Line Number "${li.line_number}", detail ${detNum}: item make is required.`);
        }
        if (!sub.item_model_id) {
          errors.push(`In Line Number "${li.line_number}", detail ${detNum}: item model is required.`);
        }
        if (!sub.item_part_id) {
          errors.push(`In Line Number "${li.line_number}", detail ${detNum}: item part is required.`);
        }
        if (typeof sub.quantity_inspected !== 'number' || sub.quantity_inspected <= 0 || sub.quantity_inspected > li.quantity_offered) {
          errors.push(`In Line Number "${li.line_number}": quantity_inspected must be > 0.`);
        }
        // serials
        // const requiredSerials = li.quantity_offered * (li.po_item_details.length);
        // const serialCount = Array.isArray(sub.item_serial_number)
        //   ? sub.item_serial_number.length
        //   : 0;
        //   if (serialCount < requiredSerials) {
        //     errors.push(
        //       `In Line Number "${li.line_number}", at least ${requiredSerials} serial numbers are required.`
        //     );
        //   }
          //console.log("serialCount", serialCount, requiredSerials);
        // if (serialCount === 0) {
        //   errors.push(
        //     `Line Item "${li.line_item_name}", detail ${detNum}: at least one serial number is required.`
        //   );
    ...
    li.po_item_details.forEach((sub, j) => {
      if (li.quantity_offered > sub.item_serial_number.length) {
        errors.push(
          `In Line Number "${li.line_number}", item: ${j + 1}: Atleast ${li.quantity_offered} serial numbers are required.`
        );
      }
    })
    // inspected-vs-serial-count mismatch
...
const isSerialNumberCorrect = (maxRequired = 0, serialNumber = 0) => {
  return serialNumber < maxRequired;
};